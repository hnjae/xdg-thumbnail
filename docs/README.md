...
Initial implementation scope is Unix-like XDG desktop environments. Cache inspection and pruning APIs may support any platform with compatible filesystem semantics, but the default `xdg-thumbnail-generate` command is Linux-specific because its default `--sandbox required` mode requires `bubblewrap` and never falls back to unsandboxed thumbnailer execution unless the user explicitly passes `--sandbox off`.
