...
Thumbnail creation, personal-cache writes, shared-repository writes, and failure entry writes are outside the project scope. Generators must live outside this library because they require source-format decoding and rendering for inputs such as PNG, WebP, documents, and video, plus orientation handling and format-specific policy that this cleanup project does not own.

The library may inspect existing PNG thumbnails and report whether their metadata, encoding, and dimensions satisfy lookup requirements. It should not provide a metadata writer, atomic save helper, thumbnail renderer, or failure-entry writer.
