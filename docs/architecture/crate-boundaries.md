...
- Expose validated payload or handle lookup as the preferred display surface for thumbnail views, returning the exact cache file content or opened file handle that passed validation together with path and metadata facts.
...
- Normalize supported caller-provided rendered PNG payloads into Freedesktop-conforming 8-bit non-interlaced RGBA PNG output before successful personal-cache installation, optionally encode explicitly described raw pixel buffers through the same final PNG path, and return the installed path plus optional final normalized payload or handle when requested.
- Install successful personal-cache thumbnails atomically from caller-provided rendered thumbnail payloads and readability-confirmed original identities, after validating the final PNG encoding and dimensions against the requested size namespace.
...
- Ask the library to normalize supported rendered PNG output to the final cache PNG format, validate successful-thumbnail namespace requirements, write required personal-cache metadata such as `Thumb::URI` and `Thumb::MTime`, write optional metadata when available, and install generated thumbnails atomically under the resolved personal thumbnail cache root. The selected thumbnailer or renderer owns aspect-ratio-preserving scaling; the generate CLI does not decode every source format to repair aspect-ratio mistakes.
