...
- Ask the library to validate the rendered payload against successful-thumbnail namespace requirements, write required personal-cache metadata such as `Thumb::URI` and `Thumb::MTime`, write optional metadata when available, and install generated thumbnails atomically under the resolved personal thumbnail cache root. The selected thumbnailer or renderer owns aspect-ratio-preserving scaling; the generate CLI initially validates dimensions and PNG conformance but does not decode every source format to repair aspect-ratio mistakes. If the generate CLI later accepts broader thumbnailer output than the core library accepts, that normalization belongs in the generate layer or a separate helper crate before core installation.
