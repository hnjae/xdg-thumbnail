...
- Validate caller-provided PNG payloads as already Freedesktop-conforming before successful personal-cache installation, and optionally encode explicitly described raw pixel buffers into 8-bit non-interlaced RGBA PNG output.
- Install successful personal-cache thumbnails atomically from caller-provided in-memory thumbnail payloads and readability-confirmed original identities, after validating the final PNG encoding and dimensions against the requested size namespace.
...
- Ask the library to validate the rendered payload against successful-thumbnail namespace requirements, write required personal-cache metadata such as `Thumb::URI` and `Thumb::MTime`, write optional metadata when available, and install generated thumbnails atomically under the resolved personal thumbnail cache root. If the generate CLI later accepts broader thumbnailer output than the core library accepts, that normalization belongs in the generate layer or a separate helper crate before core installation.
...
Executable and `TryExec` lookup should happen before entering the sandbox using desktop-entry-compatible path lookup rules. Sandbox eligibility should then check the resolved executable, resolved script interpreter if any, and literal host paths in the command template against the documented sandbox profile. If a thumbnailer explicitly names a shell in `Exec`, that shell still runs inside the sandbox and the shell entry is eligible only when the command still fits the documented profile.
