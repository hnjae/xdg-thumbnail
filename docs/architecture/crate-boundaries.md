...
- Normalize supported caller-provided rendered PNG payloads into Freedesktop-conforming 8-bit non-interlaced RGBA PNG output before successful personal-cache installation, and optionally encode explicitly described raw pixel buffers through the same final PNG path.
