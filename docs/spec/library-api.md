...
Failure entries are separate from successful thumbnail size namespaces. They are PNG metadata carriers stored under `fail/<program-version>/`; successful-thumbnail dimension limits do not apply to them. The library can locate, parse, inspect, and explicitly write failure entries, but it must not decide when an application should record a failure or suppress future generation attempts.
