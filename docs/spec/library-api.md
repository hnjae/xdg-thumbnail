...
Successful-thumbnail write APIs must normalize accepted rendered payloads to a Freedesktop-conforming final PNG before installation. Regardless of whether the caller supplies PNG bytes or a supported raw pixel buffer, the installed successful thumbnail must be an 8-bit non-interlaced RGBA PNG. PNG inputs without alpha are expanded with opaque alpha, grayscale inputs are expanded to RGB channels, grayscale-alpha inputs are expanded to RGBA, and indexed-color inputs are expanded before writing. The initial contract should reject animated PNGs, PNG inputs with unsupported color types, inputs with bit depths that cannot be converted to 8 bits per channel by the chosen PNG decoder, structurally invalid PNGs, and PNG inputs whose color-management metadata is outside the implementation's documented accepted policy. The initial accepted policy is no color conversion: inputs with no color-management chunks or explicit sRGB-compatible metadata may be accepted, while inputs that require honoring unsupported `iCCP`, standalone `gAMA`, or standalone `cHRM` semantics must be rejected rather than accepted with silently discarded color information.
