...
- Validating that saved thumbnails are 8-bit non-interlaced PNG files with full alpha support and dimensions that fit the requested size class.
...
if cache.is_valid(&cache_path, &uri, &original)? {
    return Ok(cache_path);
}

let rendered_png = render_thumbnail(path)?;
cache.save_png_atomic(&uri, CacheNamespace::Size(ThumbnailSize::Normal), rendered_png, metadata, &original)?;
```
