...
The initial stable `reason` identifiers are `already-valid`, `created`, `dry-run`, `uri-construction-failed`, `input-unreadable`, `original-metadata-unavailable`, `mime-unknown`, `no-matching-thumbnailer`, `thumbnailer-entry-invalid`, `sandbox-unavailable`, `sandbox-ineligible`, `thumbnailer-exit`, `thumbnailer-timeout`, `thumbnailer-output-missing`, `thumbnailer-output-unreadable`, `output-invalid-png`, `output-unsupported-png`, `output-dimensions-exceed-namespace`, `metadata-write-failed`, `permission-setup-failed`, and `cache-install-failed`. Additional reason identifiers require a spec update when they affect documented behavior or output compatibility.
