...
The `xdg-thumbnail-generate` CLI is an installed-thumbnailer runner for explicit local filesystem inputs. It selects and runs installed `.thumbnailer` helpers, then installs successful helper output into the personal Freedesktop thumbnail cache. It is not a cleanup tool and must not delete existing cache entries except for replacing a target thumbnail through an atomic successful regeneration.
...
`--help` and `--version` are metadata-only modes provided by the command parser. They do not require input paths and must exit successfully with code `0`. CLI help, generated man pages, and generated completions must include option descriptions matching the public behavior documented here, including that `PATH` operands are required for generation but may be omitted for completion and manpage generation.
...
When a thumbnailer exits successfully, the generate CLI verifies that the temporary output exists, is readable, and can be handed to the library as rendered thumbnail input. The library normalizes supported rendered PNG output into the final 8-bit non-interlaced RGBA cache PNG, downscales rendered output when needed to fit the requested namespace, writes metadata, creates permission-controlled temporary files, and atomically installs the result under the resolved personal thumbnail cache directory. The installed PNG must contain the personal-cache metadata required for validation, including at least `Thumb::URI` set to the cache identity URI and `Thumb::MTime`, and including `Thumb::Size` and `Thumb::Mimetype` when those values are available.

Installed thumbnails must obey the same maximum dimensions as successful cache entries in the requested namespace. Thumbnailer output may be larger than the requested namespace when it can be safely normalized and downscaled by the library. The initial generate CLI treats source aspect-ratio correctness as the renderer or thumbnailer responsibility because it does not decode every source format or compare rendered dimensions against source dimensions. Non-PNG output, invalid PNG structure, unsupported PNG features, normalization failure, resource-limit failure, final dimension violations, missing temporary output, nonzero thumbnailer exit status, timeout, metadata-write failure, permission failure, or cache-installation failure are generation failures for that input and size.
...
| Dry-run planning reaches the point where a thumbnailer command would be run in a write run                                                                                                                | `generate`        | none              |
...
| One or more invalid thumbnailer entries match a requested input-size pair, but at least one valid matching thumbnailer remains and the selected valid thumbnailer is not blocked by sandbox policy        | discovery warning | none              |
| One or more invalid thumbnailer entries match a requested input-size pair, and no valid matching thumbnailer remains after invalid entries are ignored                                                    | `skip`            | `4`               |
| Sandbox eligibility fails for the selected thumbnailer, thumbnailer execution fails, timeout occurs, output validation fails, metadata writing fails, permission setup fails, or cache installation fails | `failed`          | `1`               |
...
- `1`: one or more requested input-size pairs failed during sandbox eligibility checks for the selected thumbnailer, thumbnailer execution, output validation, metadata writing, permission setup, or cache installation.
- `2`: command-line usage error.
- `3`: thumbnailer discovery, sandbox backend setup, or cache root resolution failed before producing reliable results.
- `4`: generation completed but one or more requested input-size pairs were skipped or one or more nonfatal matching errors occurred, such as unreadable inputs, unknown MIME types, no matching thumbnailer, or invalid thumbnailer entries that leave no valid matching thumbnailer for a requested input-size pair. Invalid thumbnailer entries that do not affect requested inputs, or that are bypassed because a valid matching thumbnailer is selected, are warnings and do not contribute to the exit code.
