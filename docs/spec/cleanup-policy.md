...
Age-based cleanup needs a timestamp for comparing each thumbnail against the configured threshold. Initial behavior defaults to thumbnail file modification time because it is portable and avoids making the cleanup tool's own inspection reads affect later cleanup decisions. This default intentionally differs from the Freedesktop deletion guidance for internet-related and removable-media thumbnails, which describes age cleanup in terms of whether the thumbnail has been accessed recently. The project chooses modification time as the CLI default because access-time policy can be changed by the act of inspecting thumbnail contents on common systems. This default is intentionally more aggressive than access-time cleanup: a thumbnail that was recently read but not recently rewritten can still become an age-based deletion candidate.

Users who want cleanup to avoid deleting recently read thumbnails may explicitly choose `--age-basis access-time`. Age decisions in access-time mode must be based on timestamp metadata captured before any file reads performed by the tool, and the tool must not turn an access-time dry-run into a cache-touching operation that changes later cleanup decisions.

The prune CLI should not try to prove complex access-time semantics for every filesystem. Initial timestamp bases are `modification-time`, `access-time`, and `unavailable`. Age-based deletion in access-time mode may run only for entries whose timestamp and required inspection can be completed without making the tool's own reads change later age decisions. If access time is unavailable, cannot be preserved while reading metadata, or is identified as unreliable for the filesystem, the prune CLI reports the candidate as skipped rather than falling back silently to modification time. These skips are expected outcomes of the conservative access-time policy and do not by themselves make the command fail. Human and JSONL summaries for age-based runs must expose the selected timestamp basis and counts for timestamp-unavailable, timestamp-unreliable, and timestamp-preservation-unavailable skips. Human summaries must also point users to the default `--age-basis modification-time` behavior when access-time policy skips age-based candidates, include a concrete example such as `xdg-thumbnail-prune --older-than 30d --age-basis modification-time`, and make clear that modification time is more portable and more aggressive.

CLI help and report output must make clear that modification-time cleanup is more aggressive than access-time cleanup. When modification time is used for an age-based decision, including by default, verbose and report output must state that the decision is based on thumbnail file modification time rather than access time because many Linux systems use `relatime` or `noatime`.
...
- Modification time is the default basis for age-based cleanup. This is a documented project policy that intentionally differs from the Freedesktop deletion guidance's access-age wording for internet-related and removable-media thumbnails. Access time may be used only when the user explicitly selects it, and reports must expose the selected basis. Access-time skip summaries should tell users that modification-time cleanup is the default portable fallback.
