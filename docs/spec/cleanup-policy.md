...
Age-based cleanup needs a timestamp for "not accessed within the configured period." Initial behavior uses the thumbnail file access time when filesystem metadata exposes it. Age decisions must be based on timestamp metadata captured before any file reads performed by the tool can update that timestamp.

The CLI should not try to prove complex access-time semantics for every filesystem. Initial timestamp bases are `access-time`, `modification-time`, and `unavailable`. Age-based deletion may run for `access-time` by default. If access time is unavailable, the CLI reports the candidate as skipped rather than falling back silently to modification time. If the CLI identifies a mount or filesystem as not maintaining useful access times, it may report the timestamp as unreliable and skip age-based deletion for affected entries; otherwise it reports the captured access time as the decision basis. Human and JSONL summaries for age-based runs must expose the selected timestamp basis and counts for timestamp-unavailable or timestamp-unreliable skips.
