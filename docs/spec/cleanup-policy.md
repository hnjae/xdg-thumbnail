...
- Access time is the default basis for age-based cleanup. Modification time may be used only when the user explicitly selects it, and reports must expose that basis. Access-time skip summaries should tell users when modification-time cleanup is available as the explicit portable fallback.
