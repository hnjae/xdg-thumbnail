...
- Personal-cache entries with unreadable PNG structure, metadata required for their personal-cache context missing, or metadata with invalid syntax: delete the thumbnail when `--delete` is passed. Well-formed metadata that no longer matches an existing original is stale metadata, not invalid metadata syntax.
