...
Age-based cleanup needs a timestamp for comparing each thumbnail against the configured threshold. Initial behavior defaults to thumbnail file modification time because it is portable and can be evaluated without reading thumbnail contents through access-time-preserving filesystem operations. This default is intentionally more aggressive than access-time cleanup: a thumbnail that was recently read but not recently rewritten can still become an age-based deletion candidate.
...
- Modification time is the default basis for age-based cleanup. Access time may be used only when the user explicitly selects it, and reports must expose the selected basis. Access-time skip summaries should tell users that modification-time cleanup is the default portable fallback.
