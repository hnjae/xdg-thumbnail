...
Personal-cache thumbnails use absolute URI identities. The MD5 input for the thumbnail filename is the exact byte sequence of the accepted URI identity string after any constructor-defined URI percent-encoding. The URI used for hashing is a thumbnail identity, not a user-facing display path, renderer input path, temporary extraction path, or resolved playback path.
...
For non-local personal-cache URIs, the library must not attempt to canonicalize every possible URI scheme. Callers that own a backend such as `http:`, `smb:`, `dav:`, `trash:`, KIO-style archive URLs, or an application-specific virtual scheme choose the stable absolute original identity URI for that backend. The caller-selected identity might differ from the path or URI used internally to fetch, mount, stream, extract, or play the source; those implementation details must not replace the original identity unless the caller explicitly chooses them as the user-visible original. The caller-selected constructor must reject `file:` URIs so local file identity always goes through the library-owned absolute-path or textual local `file:` URI canonicalization APIs. The library validates that the supplied value can be an absolute thumbnail URI identity, including rejecting relative references, control characters, and unescaped non-ASCII IRI text, then preserves the accepted bytes exactly for hashing and metadata comparison. The library must not parse and reserialize caller-provided URI strings or apply scheme-specific normalization as a hidden step.

Local path URI construction percent-encodes every byte that is not safe in the canonical URI path form and emits uppercase hexadecimal escape digits. For local path bytes, the safe bytes are ASCII letters, ASCII digits, `/` as the path separator, and these path characters: `-`, `.`, `_`, `~`, `!`, `$`, `&`, `'`, `(`, `)`, `*`, `+`, `,`, `;`, `=`, `:`, and `@`. All other bytes, including space, `%`, `#`, `?`, control bytes, backslash, and non-ASCII bytes, are percent-encoded. Existing literal percent bytes in the filesystem path are encoded as `%25`; callers that already have a non-local stable URI identity should use the caller-provided URI identity API instead of passing URI text through the local path constructor.
...
Shared thumbnail repositories are scoped to the directory that contains the original. A shared thumbnail URI must be `./` followed by one canonical, minimally percent-encoded path segment for the direct child filename. Shared URI segment encoding uses the same safe bytes as local path segments except that `/` is never allowed inside the encoded filename segment.

The raw filename constructor and any textual shared-URI parser are separate APIs. The raw filename constructor accepts one direct child filename as filesystem bytes and percent-encodes literal percent bytes, so a raw Unix filename such as `dir%2Fpicture.png` is a valid single child name and becomes `./dir%252Fpicture.png`. A textual shared-URI parser must reject URI text such as `./dir%2Fpicture.png` because it decodes to a slash and would identify multiple path segments. Shared URI construction must reject empty filenames, `.` and `..`, slash path separators in raw filenames, parent segments, nested paths, textual shared URIs with encoded `/`, and any textual input that decodes to multiple path segments. The rejected forms must not be normalized into an accepted shared URI because doing so can alias different originals. On the initial Unix-like target platforms, backslash is a filename byte rather than a path separator and must be preserved through percent-encoding when needed.
...
| shared direct child with literal percent-looking text                                   | `dir%2Fpicture.png`                                              | `./dir%252Fpicture.png`                                  | `32127d4f320bca2eed708ef2a426b3cf` |
| shared filename containing literal slash or textual shared URI containing encoded slash | raw filename `dir/picture.png` or URI text `./dir%2Fpicture.png` | rejected                                                 | rejected                           |
